...
asignacion backtracking(int** matrizBeneficios, int* numNodosGenerados, int* numPasosCriterio, int* numPasosSolucion) {
    ...
    *numPasosSolucion = 0;
    ...
    do {
        Generar(nivel, s, matrizBeneficios, &bact); // Generamos una nueva asignación en el nivel actual
        if (Solucion(nivel, s, numPasosSolucion)) { // Si esta asignación es solución
        ...
        if (Criterio(nivel, s, numPasosCriterio) && (nivel < (getNumPersonas(s)-1))) { // Si la asignación actual es viable...
